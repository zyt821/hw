...
if ($_SERVER['REQUEST_URI'] === '/logout') {
    ...
    if (isset($attributes['NameID'])) {
        // Initiate SAML logout
        $logoutRequest = SAMLLogoutRequest::generate($attributes['NameID']);
        $params = ['SAMLRequest' => $logoutRequest['encoded']];
        $url = SAMLConfig::IDP_SLS_URL . '?' . http_build_query($params);
        header('Location: ' . $url);
        exit;
    } else {
        header('Location: /');
...
Location=”’ . SAMLConfig::SP_SLS_URL . ‘”/>
</md:SPSSODescriptor>
</md:EntityDescriptor>’;
?>
